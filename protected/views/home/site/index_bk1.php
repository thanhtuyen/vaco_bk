...
<div class="container"><div class="mt1"></div>
  <div class="colone">
    <?php foreach ($list_menu_left as $lm_left){ ?>
      ...
      foreach ($list_news_left as $ln_left){ 
        ...
        <div class="bocontent">
          <div class="news">
          	<?php echo CHtml::link(($language=="en") ? $ln_left->title_eng : $ln_left->title, HomeController::getUrlNews('News/list', $ln_left->id, ($language == "en") ? $ln_left->title_eng : $ln_left->title));?>
          </div>
          <div class="readmore"><?php echo CHtml::link(($language=="en") ? 'See more' : 'Xem tiếp', HomeController::getUrlNews('News/list', $ln_left->id, ($language == "en") ? $ln_left->title_eng : $ln_left->title));?></div>
        </div>
      <?php } ?>
    <?php } ?>
  </div>
  ...
  <!-- News center -->
  <?php $list_menu_center = News::model()->getMenuForHome(Menu::NOT_LIST, 0, 2); ?>
  <div class="coltwo">
    <?php 
    	//foreach ($list_menu_center as $lm_center){ 
    ...
    ?>
      ...
      <?php $news_center = Detailmenu::model()->getDetailMenu($lm_center->id); ?>
        <div class="bocontentgt">
          <p><?php echo ($language=="en") ? $news_center->caption_eng : $news_center->caption; ?></p>

          <?php if($news_center->image_path != ''):?>
            <div class="imggt">
              <?php echo CHtml::image(Yii::app()->request->baseUrl . Detailmenu::S_THUMBNAIL . $news_center->image_path,""); ?>
            </div>
          <?php endif; ?>

          <div class="readmore">
          	<?php echo CHtml::link(($language=="en") ? 'See more' : 'Xem tiếp', HomeController::getUrl('Detailmenu/view', $news_center->id, ($language == "en") ? $news_center->title_eng : $news_center->title)); ?>
          </div>
		</div>
    <?php 
    		}
  ...
  <div class="colthree">
    <?php foreach ($list_menu_right as $list_menu_right){ ?>
      ...
      <?php
      $list_news_right = News::model()->getListNews($list_menu_right->id, true);
      ...
      foreach ($list_news_right as $ln_right){ $num_current++; ?>
        <div class="bocontent">
          ...
          <div class="readmore"><?php echo CHtml::link(($language=="en") ? 'See more' : 'Xem tiếp', HomeController::getUrlNews('News/list', $ln_right->id, ($language == "en") ? $ln_right->title_eng : $ln_right->title));?></a></div>
