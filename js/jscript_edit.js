...
if( typeof(LofArticleSlideshow) == 'undefined' ){
  var LofArticleSlideshow = new Class( {
    initialize:function( _lofmain, options ){ 
      ...
      this.descriptions = [];
      var eMain      = _lofmain.find('.lof-main-wapper'); //var eMain      = _lofmain.getElement('.lof-main-wapper');
      var eNavigator = _lofmain.find('.lof-navigator-outer .lof-navigator'); // var eNavigator = _lofmain.getElement('.lof-navigator-outer .lof-navigator');
      var eNavOuter  = _lofmain.find('.lof-navigator-outer'); // var eNavOuter  = _lofmain.getElement('.lof-navigator-outer');
      this.currentNo  = 0; 
      this.nextNo     = null;
      ...
      this.onClick = false;
      if( $defined(eMain) ){
        this.slides    = eMain.find( this.setting.mainItemSelector ); // this.slides    = eMain.getElements( this.setting.mainItemSelector );
        ...
        this.slides.each( function(item, index) { 
		  ...
		  if( item.find(".lof-description") ) { // if( item.getElement(".lof-description") ) {
				this.descriptions[index] = new Fx.Morph(item.find(".lof-description")); // item.getElement(".lof-description")
	  ...
	  if( $defined( _lofmain.find(".lof-startstop") ) ) { // $defined( _lofmain.getElement(".lof-startstop") )
					_lofmain.find(".lof-startstop").addEvent('click', function(){ // _lofmain.getElement(".lof-startstop")
						if( _lofmain.find(".lof-startstop div").hasClass("lof-stop") ){ // _lofmain.getElement(".lof-startstop div")
							this.setting.auto =true;
							this.play( this.setting.interval,'next', true );
							 _lofmain.find(".lof-startstop div").removeClass("lof-stop").addClass("lof-start"); // _lofmain.getElement(".lof-startstop div")
						}else {
							this.setting.auto =false;
							this.stop();
							 _lofmain.find(".lof-startstop div").removeClass("lof-start").addClass("lof-stop"); // _lofmain.getElement(".lof-startstop div")	
						}
					}.bind(this) );
				} 
         if(  this.setting.auto ){
      ...
      if( $defined(eNavigator) && $defined(eNavOuter) ){
        ...
        this.navigatorItems = eNavigator.find( this.setting.navSelector ); // this.navigatorItems = eNavigator.getElements( this.setting.navSelector );
        if( this.setting.navItemsDisplay > this.navigatorItems.length ){
          this.setting.navItemsDisplay = this.navigatorItems.length;  
        }
        
        if( $defined(_lofmain.find(".lof-bullets"))  ){ // $defined(_lofmain.getElement(".lof-bullets"))
    ...
    start:function( isStart, _lofmain ){
	 ...
	 var obj =  _lofmain.find('.preload'); // var obj =  _lofmain.getElement('.preload');
      ...
      if( obj ) {
        ...
        if( this.setting.auto && this.slides.length > 1 ){
		 ...
		 if( $defined( _lofmain.find(".lof-startstop") ) ) { // if( $defined( _lofmain.getElement(".lof-startstop") ) )
				if( this.setting.auto ){
					_lofmain.find(".lof-startstop div").addClass("lof-start"); // _lofmain.getElement(".lof-startstop div")
				} else {
					_lofmain.find(".lof-startstop div").addClass("lof-stop");	// _lofmain.getElement(".lof-startstop div")
				}
		}
    ...
    preloadImages:function( obj ){  
      ...
      var counter=0;
      var self = this;
      var _length = this.slides.find('img').length; // var _length = this.slides.getElements('img').length;
      ...
      this.slides.find('img').each( function(img, index){ // this.slides.getElements('img').each( function(img, index){
        image = new Image();
        image.src=img.src;
        if( !image.complete ){          
